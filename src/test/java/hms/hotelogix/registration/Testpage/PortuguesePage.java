...
public class PortuguesePage {
	...
	public void launchHotelogixDotCom() throws Exception{
		try{
			...
			GenericMethods.GI().driver.get("http://www.hotelogix.com/portuguese/");
	...
	public void fn_HotelRegister() throws Exception{
		...
		hobj.fn_TryforPortugueseHotel(HM.get("HotelName_ED"));
	    ...
	    SP.fn_CheckMailandClick(HM.get("View_Link"), HM.get("P_Subject_TX"));
	...
	public void fn_WittyWizardPages() throws Exception{
		try{
			...
			wi2.fn_ClickonTestDrive("Hotelogix_WittyWizardPage3_Portuguese_TestDriveNow_BT");
			...
			slp.fn_CongratulationMailLink(HM.get("PortugueseEmailSubject_TX"));
			slp.fn_GetHotelInfo();
			System.out.println(" Hotel Code: "+ slp.HotelID + " User Name: " +slp.User_Name + " Password: "+ slp.Password);
			Reporter.log("Portuguese Hotel Credential");
			...
			logger.log(LogStatus.PASS,"<b>Portuguese Hotel</br> Registration Completed</b>","<b>"+ slp.HotelID  +"</b></br>" +"<b>"+  slp.User_Name + "</b><br>"+"<b>"+slp.Password +"</b><br>");
			ER.endTest(logger);
			ER.flush();
